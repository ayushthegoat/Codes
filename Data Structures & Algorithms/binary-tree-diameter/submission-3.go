...
// so the intution is very simply i will fidn the height simpyl but which finding 
// the heing i know i visint every sinlge node at each node i wil check with a
//  golabl variable that okay sir if the heingt of the left and reight subtree
//   from thsi node is actualy larger that what i curerntly have as the largest right ?
var d int 
func diameterOfBinaryTree(root *TreeNode) int {
    d = 0
    if root == nil {
